...
def deleateZielbestand(session: requests.Session, productID: str, soup=None) -> BeautifulSoup:
    ...
    currentURL = f"https://erp.digitecgalaxus.ch/de/Product/Availability/{productID}"
    ...
    try:
        ...
        for tr in tr_elements:
            ...
            if tr_id and "ProductSiteTargetInventoryOverrideTable_row_" in tr_id:
                ...
                delete_url = f"https://erp.digitecgalaxus.ch/de/ProductSiteTargetInventoryOverride/TableDelete/{rule_id}"
...
def addZielbestand(session: requests.Session, productID: str, from_date: str, to_date: str, product_quantity: int, filialen: List[str], soup=None) -> BeautifulSoup:
    ...
    try:
        ...
        if mandant is not None:
            create_url = "https://erp.digitecgalaxus.ch/ProductSiteTargetInventoryOverride/TableNewPost/" + mandant
        else:
...
# Higher level function to update the Target stock of a product, returns a dictionary of how many products will be transfered to each filiale in the filialen (except Wohlen, Winterthur and Genf)
...
def updateZielbestand(session: requests.Session, productID: str, date_start: str, date_end:str, quantity: int, filialen: List[str] = ["St. Gallen"]) -> Dict[str, int]:
    ...
    # Print timestamp of the updating time
    print(f"Updating at {pd.Timestamp.now().strftime('%Y-%m-%d %H:%M:%S')}")
